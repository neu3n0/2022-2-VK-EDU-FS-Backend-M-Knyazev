...
from .serializers import ChatSerializer, ChatMemberSerializer
...
def chat_list2(request, user_id):
    user = get_object_or_404(User, pk=user_id)
    chat_members = ChatMember.objects.filter(user=user)
    data = ChatMemberSerializer(chat_members, many=True).data
